...
@Service
public class CartServiceDelegateV3 {
	@Autowired
	...
	*/
	...
	@return Map<String,Object> 购物车相关信息（含赠品）
	*/
	public Map<String,Object> viewCart(String token,String zipcode)throws Exception{
		Map<String,Object> result = new HashMap<String,Object>();
		Gson gson = new Gson(); 
		Token tokenIn = gson.fromJson( StringUtil.decode(token), Token.class);
		...
		//1、提取购物车信息
			List<Cart> lstCart = cartRedisService.selectCartsForRedis(tokenIn);
			List<Cart> lstCartError = new ArrayList<Cart>();
			//1.2过滤区域限购商品
			Map<String,List<Cart>> mapFilterCart = this.filterCart(lstCart, zipcode);
			lstCart = (List<Cart>)mapFilterCart.get("lstCartCorrect");
			lstCartError = (List<Cart>)mapFilterCart.get("lstCartError");
			...
			if(lstCart.size()==0){
				  ...
				  result.put("token", token);	
				  Map<String,Object> mapErrorItems = new HashMap<String,Object>();
				  List<Object> lstErrorItems = new ArrayList<Object>();
				  for(Cart cartError:lstCartError){
					  mapErrorItems = new HashMap<String,Object>();
					  mapErrorItems.put("gid", cartError.getGoods_id());
					  mapErrorItems.put("name", cartError.getGoods_name());
					  mapErrorItems.put("ename", cartError.getGoods_ename());
					  mapErrorItems.put("price", StringUtil.formatPrice(cartError.getGoods_price()));
					  mapErrorItems.put("number", cartError.getGoods_number());
					  BigDecimal goodsNum = new BigDecimal(new BigInteger(cartError.getGoods_number().toString()));
					  mapErrorItems.put("subtotal", StringUtil.formatPrice(cartError.getGoods_price().multiply(goodsNum)));
					  mapErrorItems.put("currency", "$");
					  mapErrorItems.put("is_gift", cartError.getIs_gift());
					  mapErrorItems.put("image", YamiConstant.IMAGE_URL+cartError.getGoods_thumb());
					  if((!cartError.getIsOnSale())||cartError.getIsDelete()){
						  mapErrorItems.put("is_oos", 1);  
					  }else{
						  mapErrorItems.put("is_oos", 0); 
					  }
					  mapErrorItems.put("is_limited", cartError.getIsLimited()?1:0); 
					  mapErrorItems.put("limited_number", cartError.getLimitedNumber()); 
					  mapErrorItems.put("limited_quantity",  cartError.getLimitedQuantity()); 
					  lstErrorItems.add(mapErrorItems);
				  }
				  ...
				  result.put("errorItems", lstErrorItems);	
...
//			  for(OrderGenerate orderGenerate:lstOrderGenerate){
		...
		//5、返回数据整合处理
			result = integrateResultV3(token,lstOrderGenerate,lstCart,lstCartError,bonusType,null,null,0);	   
		return result;
	}
	/*
	 结算页面版本2.0
...
//			  for(OrderGenerate orderGenerate:lstOrderGenerate){
	...
	/*
	 ...
	 @return Boolean	 
	*/
	public Map<String,Object> checkoutV3(String token)throws Exception{
...
//			  for(OrderGenerate orderGenerate:lstOrderGenerate){
		...
		//5、整合返回值		
				result = integrateResultV3(token,lstOrderGenerate,lstCart,null,bonusType,taxLookup,users,1);	  
		return result;
...
//			  if(null!=orderGenerate.getBonusId()&&bonusType.getIsPercentOff()==1){
	  ...
	  @return Map<String,Object> 放回结果信息集合
	 */
	  public Map<String,Object> integrateResultV3(String token,List<OrderGenerate> lstOrderGenerate,List<Cart> lstCart,List<Cart> lstCartError,BonusType bonusType,TaxLookup taxLookup,Users users,int fun_flag)throws Exception{
		  Map<String,Object> result = new HashMap<String,Object>();
...
//			  for(Cart cart:lstCart){
			  ...
			  mapVendor.put("vendor_ename", orderGenerate.getVendors().getVendorEname());
			  BigDecimal vendor_subtotal = BusinessComputing.accountOrderByVendorId(lstCart, orderGenerate.getVendorId());
			  mapVendor.put("vendor_subtotal", StringUtil.formatPrice(vendor_subtotal.setScale(2,BigDecimal.ROUND_HALF_UP)));
			  if(orderGenerate.getVendors().getVendorId().intValue()==Integer.parseInt(YamiConstant.VENDOR_ID_YAMIBUY)){
				  yamibuySubtotal = vendor_subtotal;
			  }
			  mapVendor.put("currency", "$");
...
//			  if(null!=orderGenerate.getBonusId()&&bonusType.getIsPercentOff()==1){
				  ...
				  mapBonus.put("bonus_ename", bonusType.getTypeEdesc());
				  discountVendor = BusinessComputing.countPromoPriceByVendor(bonusType, lstCart, orderGenerate.getVendors()).getComputerPrice().setScale(2,BigDecimal.ROUND_HALF_UP);
				  mapBonus.put("discount", StringUtil.formatPrice(discountVendor));
				  mapBonus.put("currency", "$");
				  if(discountVendor.compareTo(new BigDecimal(0.0))>0){
					  mapVendorCart.put("bonus", mapBonus);
				  }
				  ...
				  mapVendor.put("vendor_subtotal", StringUtil.formatPrice(vendor_subtotal.subtract(discountVendor).setScale(2,BigDecimal.ROUND_HALF_UP)));
				  mapVendor.put("before_vendor_subtotal", StringUtil.formatPrice(vendor_subtotal.setScale(2,BigDecimal.ROUND_HALF_UP)));
			  }
			  //结算页面方法调用才统计该部分信息数据
			  if(fun_flag==1||fun_flag==2){
				  //1.4配送方式信息处理
		  ...
		  //2.1商品原价金额合计
		  BigDecimal subtotal = BusinessComputing.accountOrder(lstCart);
		  mapAmount.put("subtotal", StringUtil.formatPrice(subtotal.setScale(2,BigDecimal.ROUND_HALF_UP)));
		  //2.2折扣码
		  BigDecimal discount = new BigDecimal(0.00);
		  String bonus_sn= "";
		  ComputerResult computerResult4Dealprice = new ComputerResult();
		  ...
		  if(discount.compareTo(new BigDecimal(0.0))==0){
			  cancelCode(token);
			  mapAmount.put("bonus_sn", "");
			  mapAmount.put("discount", StringUtil.formatPrice(discount.setScale(2,BigDecimal.ROUND_HALF_UP)));
		  }else{
			  mapAmount.put("bonus_sn", bonus_sn);
			  mapAmount.put("discount", StringUtil.formatPrice(discount.setScale(2,BigDecimal.ROUND_HALF_UP)));
		  }
		  ...
		  if(fun_flag==1||fun_flag==2){
			  //2.4运费合计值
			  mapAmount.put("shipping", StringUtil.formatPrice(shippingFeeTotal));
			  //2.5扣税合计值
			  ComputerResult computerResult4Tax = new ComputerResult();
			  if(null!=userAddress){
				  computerResult4Tax = BusinessComputing.countTax(users.getUserId(), lstCart, userAddress, taxLookup, computerResult4Dealprice.getComputerGoodResult());
				  tax = computerResult4Tax.getComputerPrice();
				  mapAmount.put("tax", StringUtil.formatPrice(tax));
			  }
			  //2.6积分使用值,只能YAMIBUY自营商品金额
			  int pointDisplay = 0;
			  if(users.getPayPoints()> yamibuySubtotal.multiply(new BigDecimal(100)).intValue()){
			  ...
			  mapAmount.put("pointDisplay", pointDisplay);
			  mapAmount.put("pointFlag", pointFlag);
			  mapAmount.put("points", StringUtil.formatPrice(point.setScale(2,BigDecimal.ROUND_HALF_UP)));
		  }
		  //2.7最终金额合计
		  BigDecimal amount = subtotal.add(shippingFeeTotal).add(tax).subtract(point).subtract(discount);
		  mapAmount.put("amount", StringUtil.formatPrice(amount.setScale(2,BigDecimal.ROUND_HALF_UP)));
		  mapAmount.put("currency", "$");
		  result.put("total", mapAmount);	
		  result.put("vendorCart", lstVendorCart);	
		  
		  if(null!=lstCartError){
			  for(Cart cartError:lstCartError){
				  mapErrorItems = new HashMap<String,Object>();
				  mapErrorItems.put("gid", cartError.getGoods_id());
				  mapErrorItems.put("name", cartError.getGoods_name());
...
//		}
		...
		@return Map<String,Object> 
		*/	
		private  Map<String,List<Cart>> filterCart(List<Cart> lstCart,String zip){

			Map<String,List<Cart>> result = new HashMap<String,List<Cart>>();
			List<Cart> lstCartCorrect = new ArrayList<Cart>();
			List<Cart> lstCartError = new ArrayList<Cart>();
			...
			for(Cart cart:lstCart){
				...
				if(cart.getIsDistrict()==0||DistrictCheck.goodsDistrictCheck(zip, lstShopZipcodeLimit)){
					lstCartCorrect.add(cart);
				}
				else{
					lstCartError.add(cart);
					
				}
			}
			result.put("lstCartCorrect", lstCartCorrect);
			result.put("lstCartError", lstCartError);
			return result;
		...
		获取邮寄方式总LIST(版本3.0)
		 ...
		 @return List<Shipping> 
		*/
		public Map<String,Object>  getShippingListV3(String token)throws Exception{
			Gson gson = new Gson(); 
			Token tokenIn = gson.fromJson( StringUtil.decode(token), Token.class);
			String defaultAdd_zip = null;
			if(tokenIn.getIsLogin()==1){
				List<OrderGenerate> lstOrderGenerate =  orderGenerateRedisService.selectAllForOrderGenerate(tokenIn);
				for(OrderGenerate orderGenerate:lstOrderGenerate){
					if(null!=orderGenerate.getUserAddress()){
					defaultAdd_zip = orderGenerate.getUserAddress().getZipcode();
					}
				}
			...
			for(String key:mapVendorTemp.keySet()){
				...
				for(Shipping shipping:lstShippingTemp){
					if(key.equals(shipping.getVendorId().toString())){
						...
						//支持GESOO配货方式业务逻辑处理
						editGesooItem = DistrictCheck.shippingDistrictCheck(defaultAdd_zip, shipping.getLstShippingDistrictZipcode());
						if(shipping.getShippingId().intValue()==YamiConstant.SHIPPING_ID_GESOO){
							
							if(editGesooItem){
								mapShipping.put("is_primary", 1);
								lstShipping.add(mapShipping);
							}
						}else{
							if(editGesooItem){
								mapShipping.put("is_primary", 0);
							}
							lstShipping.add(mapShipping);	
						}
					}
		...
		获取邮寄方式LIST(版本3.0)
		 ...
		 @return List<Shipping> 
		*/
		public Map<String,Object>  getVendorShippingV3(String token,int vendor_id)throws Exception{
			Gson gson = new Gson(); 
			Token tokenIn = gson.fromJson( StringUtil.decode(token), Token.class);
			Map<String,Object> result = new HashMap<String,Object>();
			Map<String,Object> mapShipping = new HashMap<String,Object>();
		...
		/*
	 	获取用户购物车不符合指定ZIP的区域限购商品信息
	 	 @param token		
	 	 @return List<Shipping> 
	 	*/
	 	public Map<String,Object>  getErrorItemsOfCart(String token,String zipcode)throws Exception{
	 		Map<String,Object> result = new HashMap<String,Object>();
	 		Gson gson = new Gson(); 
			Token tokenIn = gson.fromJson( StringUtil.decode(token), Token.class);
	 		...
	 		List<Cart> lstCart = cartRedisService.selectCartsForRedis(tokenIn);
	 		 List<OrderGenerate> lstOrderGenerate= orderGenerateRedisService.selectAllForOrderGenerate(tokenIn);
	 			...
	 			if(null==zipcode||"".equals(zipcode)){
	 				if(null!=lstOrderGenerate.get(0).getUserAddress()){
	 					zipcode = lstOrderGenerate.get(0).getUserAddress().getZipcode();
	 				}
	 			}
	 		for(Cart cart:lstCart){
	 			List<ShopDistrictZipcode> lstShopZipcodeLimit = goodsSeviceDelegate.getGoodsZipLimit(cart.getGoods_id());
	 			//1.1.1、区域限购商品判断
	 			if(!DistrictCheck.goodsDistrictCheck(zipcode, lstShopZipcodeLimit)&&cart.getIsDistrict().intValue()==1){
 				  mapItems = new HashMap<String,Object>();
 				  mapItems.put("gid", cart.getGoods_id());
 				  mapItems.put("name", cart.getGoods_name());
 				  mapItems.put("ename", cart.getGoods_ename());
 				  mapItems.put("price", StringUtil.formatPrice(cart.getGoods_price()));
 				  mapItems.put("number", cart.getGoods_number());
				  BigDecimal goodsNum = new BigDecimal(new BigInteger(cart.getGoods_number().toString()));
				  mapItems.put("subtotal", StringUtil.formatPrice(cart.getGoods_price().multiply(goodsNum)));
				  mapItems.put("currency", "$");
				  mapItems.put("is_gift", cart.getIs_gift());
				  mapItems.put("image", YamiConstant.IMAGE_URL+cart.getGoods_thumb());
				  if((!cart.getIsOnSale())||cart.getIsDelete()){
					  mapItems.put("is_oos", 1);  
				  }else{
					  mapItems.put("is_oos", 0); 
				  }
				  mapItems.put("is_limited", cart.getIsLimited()?1:0); 
				  mapItems.put("limited_number", cart.getLimitedNumber()); 
				  mapItems.put("limited_quantity",  cart.getLimitedQuantity()); 
				  mapItems.put("error_info_cn",  ErrorCodeEnum.ER1117.getCMsg()); 
				  mapItems.put("error_info_en",  ErrorCodeEnum.ER1117.getEMsg()); 
				  lstItems.add(mapItems);
	 			}
				//1.1.2、下架、删除、购买数量判断
					  ...
					  mapItems.put("limited_number", cart.getLimitedNumber()); 
					  mapItems.put("limited_quantity",  cart.getLimitedQuantity()); 
					  mapItems.put("error_info_cn",  ErrorCodeEnum.ER1101.getCMsg()); 
					  mapItems.put("error_info_en",  ErrorCodeEnum.ER1101.getEMsg()); 
					  lstItems.add(mapItems);
				}  
				//1.1.3库存判断,库存不满足
				else if(cart.getGoods_number_stock()<cart.getGoods_number()){
					  ...
					  mapItems.put("limited_number", cart.getLimitedNumber()); 
					  mapItems.put("limited_quantity",  cart.getLimitedQuantity()); 
					  mapItems.put("error_info_cn",  ErrorCodeEnum.ER1103.getCMsg()); 
					  mapItems.put("error_info_en",  ErrorCodeEnum.ER1103.getEMsg()); 
					  lstItems.add(mapItems);
					}
	 		}
	 	...
	 	/*
	 	将购物车中下架、库存不足、区域限购商品移到收藏夹
	 	 @param token		
	 	 @return List<Shipping> 
	 	*/
	 	public Map<String,Object>  moveCartErrorItemsToFav(String token,String zipcode)throws Exception{
	 		Map<String,Object> result = new HashMap<String,Object>();
	 		Gson gson = new Gson(); 
			Token tokenIn = gson.fromJson( StringUtil.decode(token), Token.class);
			int zip = 0;
	 		...
	 		for(Cart cart:lstCart){
	 			List<ShopDistrictZipcode> lstShopZipcodeLimit = goodsSeviceDelegate.getGoodsZipLimit(cart.getGoods_id());
	 			if(!DistrictCheck.goodsDistrictCheck(zipcode, lstShopZipcodeLimit)&&cart.getIsDistrict().intValue()==1){
	 				transactionDelegate.transactionAddToFavorites(token,cart.getGoods_id());
	 				quantityChange(token,cart.getVendor_id(),cart.getGoods_id(),0);
	 			}
				//1.1.2、下架、删除、购买数量判断
	 			else if (!cart.getIsOnSale()||cart.getIsDelete()||cart.getGoods_number().intValue()<=0){
