...
@Service
@Path("/cartV2")
public class RestCartServiceV2 {
	@Autowired
	...
	public Response miniCart(@QueryParam("token") String token)throws Exception {
		Map<String,Object> result = cartServiceDelegateV2.miniCart(token);
		return Response.status(Status.OK).entity(result).build();
	...
	@Path("/viewCart")
	@Produces("application/json;charset=utf-8")
	public Response viewCart(@QueryParam("token") String token)throws Exception {
		Map<String,Object> result = cartServiceDelegateV2.viewCart(token);
		return Response.status(Status.OK).entity(result).build();
	...
	public Response checkout(@QueryParam("token") String token)throws Exception{
		...
		Map<String,Object> result = cartServiceDelegateV2.checkout(token);
		return Response.status(Status.OK).entity(result).build();
	...
	public Response getShippingList(@QueryParam("token") String token)throws Exception{
		...
		Map<String,Object> result = cartServiceDelegateV2.getShippingList(token);
		return Response.status(200).entity(result).build();
	...
	public Response getVendorShipping(@QueryParam("token") String token,@QueryParam("vendor_id") int vendor_id)throws Exception{
		...
		Map<String,Object> result = cartServiceDelegateV2.getVendorShipping(token,vendor_id);
		return Response.status(200).entity(result).build();
	...
	public Response getVendorShippingFreeAmount(@QueryParam("token") String token,@QueryParam("vendor_id") int vendor_id)throws Exception{
		...
		Map<String,Object> result = cartServiceDelegateV2.getVendorShippingFreeAmount(token,vendor_id);
		return Response.status(200).entity(result).build();
